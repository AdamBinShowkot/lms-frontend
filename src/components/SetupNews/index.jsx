...
const ContentMain=({isUpdate})=>{
    ...
    useEffect(()=>{
       if(isUpdate){
        form.setFieldsValue({
            ...
            news:{
                value:2,
                label:"Aliquam eget pharetra metus.",
                key:2
            }
        })
    ...
    const handleFormFinish=(state)=>{
        console.log("News Setup Data: ",state);
    }

    const handleFormOnChange=(changeValue,allValue)=>{
        // console.log(changeValue)
        try{
            if(changeValue.news){
                let description="";
                newsLists.map((data)=>{
                    if(data.id===changeValue.news){
                        description=data.description
                    }
                })
                setMyData(description)
    ...
    try{
        newsLists.map((data)=>{
            ...
            newsListsConfig=[...newsListsConfig,obj]
        })
    }catch{
